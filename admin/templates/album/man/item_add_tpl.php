...
<form name="supplier" id="validate" class="form" action="index.php?com=album&act=save<?php if($_REQUEST['type']!='') echo'&type='. $_REQUEST['type'];?>" method="post" enctype="multipart/form-data">
	<div class="widget">
		...
		<?php if($config_truongkhac=="true"){?>
		<div class="formRow lang_hidden lang_vi active">
			<label>Tên 2</label>
			<div class="formRight">
                <input type="text" name="name_vi" title="Nhập Tên 2" id="name_vi" class="tipS" value="<?=@$item['name_vi']?>" />
			</div>
			<div class="clear"></div>
		</div>
	
		<div class="formRow lang_hidden lang_en">
			<label>Tên 2 (Tiếng anh)</label>
			<div class="formRight">
                <input type="text" name="name_en" title="Nhập Tên 2" id="name_en" class="tipS" value="<?=@$item['name_en']?>" />
			</div>
			<div class="clear"></div>
		</div>
       	<?php } ?>
