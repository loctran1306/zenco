...
<script type="text/javascript">		
	...
	$(document).ready(function() {
		...
		$('.update_mau').change(function(event) {
			var id = $(this).attr('rel');
			var table = 'product_photo';
			var value = $(this).val();
			$.ajax ({
				type: "POST",
				url: "ajax/update_mau.php",
				data: {id:id,table:table,value:value},
				success: function(result) {
				}
			});
...
function get_main_sub()
  ...
  //------------ tags-------------------------
    if($item['tags']!=''){
		...
		$sql = "select id,ten_vi from #_tags where type='".$_GET['type']."' and id<>".$tags[0];
		for ($i=1,$count = count($tags); $i < $count ; $i++) { 
			$sql .=" and id<> ".$tags[$i];
		}
	}else{
		$sql = "select id,ten_vi from #_tags where type='".$_GET['type']."'";
	}
	$d->query($sql);
...
<form name="supplier" id="validate" class="form" action="index.php?com=product&act=save<?php if($_REQUEST['type']!='') echo'&type='. $_REQUEST['type'];?>" method="post" enctype="multipart/form-data">
	<div class="widget">
		...
		<div class="formRow">
			...
			<div class="mt10"><img src="<?=_upload_product.$item['thumb2']?>"  alt="NO PHOTO" width="100" /></div>
		...
		<div class="formRow">
			<label>Mã sp </label>
			<div class="formRight">
                <input type="text" name="masp" title="Nhập mã sp" id="masp" class="tipS" value="<?=@$item['masp']?>" />
			</div>
			<div class="clear"></div>
		</div>
		...
		<div class="formRow">
			<label>Giá</label>
			<div class="formRight">
                <input type="text" name="giaban" title="Nhập giá" id="giaban" class="conso tipS" value="<?=@$item['giaban']?>" />
			</div>
			<div class="clear"></div>
		</div>
		<!--
                ...
                item: '<li class="jFiler-item">\
                            <div class="jFiler-item-container">\
                                <div class="jFiler-item-inner">\
                                    ...
                                    </div>\<input type="text" name="stthinh[]" placeholder="Nhập STT" />\
                                </div>\
                            </div>\
                        </li>',
                itemAppend: '<li class="jFiler-item">\
                            <div class="jFiler-item-container">\
                                <div class="jFiler-item-inner">\
                                    ...
                                    </div>\<input type="color" name="stthinh[]" placeholder="Nhập Màu" />\
                                </div>\
                            </div>\
                        </li>',
