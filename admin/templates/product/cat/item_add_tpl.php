...
<form name="supplier" id="validate" class="form" action="index.php?com=product&act=save_cat<?php if($_REQUEST['type']!='') echo'&type='. $_REQUEST['type'];?>" method="post" enctype="multipart/form-data">
	<div class="widget">
		...
		<?php if($config_khac =='true') {?>
		<div class="formRow lang_hidden lang_vi active">
			<label>Tiêu đề 2</label>
			<div class="formRight">
                <input type="text" name="name_vi" title="Nhập tiêu đề 2" id="name_vi" class="tipS validate[required]" value="<?=@$item['name_vi']?>" />
			</div>
			<div class="clear"></div>
		</div>
		<div class="formRow lang_hidden lang_en">
			<label>Tiêu đề 2 (Tiếng anh)</label>
			<div class="formRight">
                <input type="text" name="name_en" title="Nhập tiêu đề 2" id="name_en" class="tipS validate[required]" value="<?=@$item['name_en']?>" />
			</div>
			<div class="clear"></div>
		</div>
		<?php } ?>
