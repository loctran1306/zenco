...
function get_phuongs(){
	...
	if($_REQUEST['id_quan']!='')
	{
	$where.=" and  id_quan=".$_REQUEST['id_quan']."";
	$link_add .= "&id_quan=".$_GET['id_quan'];
	}
	...
	$url = "index.php?com=tinhthanh&act=man_phuong&".$link_add."";
	$paging = pagination($where,$per_page,$page,$url);
...
function get_list($id=0){
	global $d, $list;
	$sql = "select * from #_tinh order by ten asc";
	$d->query($sql);
	$cats = $d->result_array();
