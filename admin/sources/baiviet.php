...
function save_man(){
	...
	$file_name1=images_name($_FILES['file_download']['name']);
	...
	if($id){
		...
		if($photo2 = upload_image("file2", 'jpg|png|gif|JPG|jpeg|JPEG', _upload_baiviet,$file_name2)){
			$data['photo2'] = $photo2;	
			$data['thumb2'] = create_thumb($data['photo2'], _width_thumb2, _height_thumb2, _upload_baiviet,$file_name2,_style_thumb);			
			$d->setTable('baiviet');
			$d->setWhere('id', $id);
			$d->select();
			if($d->num_rows()>0){
				$row = $d->fetch_array();
				delete_file(_upload_baiviet.$row['photo2']);	
				delete_file(_upload_baiviet.$row['thumb2']);					
			}
		}

		if($file_download = upload_image("file_download", 'pdf|doc|docx|rar|zip|xls|xlsx', _upload_baiviet,$file_name1)){
			...
			if($d->num_rows()>0){
				$row = $d->fetch_array();
				delete_file(_upload_baiviet.$row['file_download']);		
			}
		}
		...
		if($file_download = upload_image("file_download", 'pdf|doc|docx|rar|zip|xls|xlsx', _upload_product,$file_name1)){
			$data['file_download'] = $file_download;	
		}
...
function delete_man(){
	...
	if(isset($_GET['id'])){
		...
		$d->reset();
		$sql = "select id,photo,thumb,thumb2,photo2 from #_baiviet where id='".$id."'";
		$d->query($sql);
		if($d->num_rows()>0){
			...
			while($row = $d->fetch_array()){
				delete_file(_upload_baiviet.$row['photo']);
				delete_file(_upload_baiviet.$row['thumb']);
				delete_file(_upload_baiviet.$row[$i]['photo2']);
				delete_file(_upload_baiviet.$row[$i]['thumb2']);
			}
...
function get_lists(){
	...
	$sql = "select ten_vi,id,stt,noibat,hienthi from $where $limit";
	$d->query($sql);
	$items = $d->result_array();
...
function save_list(){
	...
	if($id){
		...
		if($photo2 = upload_image("file2", 'jpg|png|gif|PNG|GIF|JPG|JPEG|jpeg', _upload_baiviet,$file_name2)){
			$data['photo2'] = $photo2;
			$data['thumb2'] = create_thumb($data['photo2'], _width_thumb2, _height_thumb2, _upload_baiviet,$file_name2,_style_thumb);	
			$d->setTable('baiviet_list');
			$d->setWhere('id', $id);
		...
		if($photo2 = upload_image("file2", _img_type, _upload_baiviet,$file_name2)){
			$data['photo2'] = $photo2;
			$data['thumb2'] = create_thumb($data['photo2'], _width_thumb2, _height_thumb2, _upload_baiviet,$file_name2,_style_thumb);	
		}
		$data['ten_vi'] = $_POST['ten_vi'];
...
function delete_list(){
	...
	if(isset($_GET['id'])){
		$id =  themdau($_GET['id']);
		$d->reset();
		$sql = "select id,photo,thumb,thumb2,photo2 from #_baiviet_list where id='".$id."'";
		$d->query($sql);
		if($d->num_rows()>0){
			while($row = $d->fetch_array()){
				delete_file(_upload_baiviet.$row['photo']);
		...
		for ($i=0 ; $i<count($listid) ; $i++){
			...
			$id =  themdau($idTin);		
			$d->reset();
			$sql = "select id,photo,thumb,photo2,thumb2 from #_baiviet_list where id='".$id."'";
			$d->query($sql);
			if($d->num_rows()>0){
				while($row = $d->fetch_array()){
					delete_file(_upload_baiviet.$row['photo']);
